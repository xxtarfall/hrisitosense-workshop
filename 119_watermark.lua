...
local watermark_items = {
	...
	get_width = function(self, frame_data)
		...
		self.plus_width = renderer_measure_text(nil, "+")
		self.alpha_width = renderer_measure_text(nil, " [alpha]")
		return self.game_width + self.sense_width + self.plus_width + self.alpha_width
	end,
	draw = function(self, x, y, w, h, r, g, b, a)
		-- Define custom colors
		local r_sense, g_sense, b_sense = ui_get(color_reference)               -- main color
		local r_plus, g_plus, b_plus = 0, 255, 150                              -- bright greenish +
		local r_alpha, g_alpha, b_alpha = 255, 90, 90                           -- reddish [alpha]

		-- Draw each part with its style
		renderer_text(x, y, 255, 255, 255, a, nil, 0, "game")                   -- white
		renderer_text(x + self.game_width, y, r_sense, g_sense, b_sense, a, nil, 0, "sense") -- custom color
		renderer_text(x + self.game_width + self.sense_width, y, r_plus, g_plus, b_plus, a, nil, 0, "+") -- greenish +
		renderer_text(x + self.game_width + self.sense_width + self.plus_width, y, r_alpha, g_alpha, b_alpha, a, nil, 0, " [alpha]") -- red [alpha]
	end
	},
	{
		name = "Custom text",
